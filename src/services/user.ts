...
export async function createUserFromLogin(state: string, code: string) {
  ...
  await bot.telegram.sendMessage(user.telegramId, `You're now logged in! We found ${totalItems} contacts.`);

  return user;
}

export async function contactsForUser(user: User.JSONSelectable, filter?: (contact: Contact) => boolean) {
...
export async function birthdaysForUser(user: User.JSONSelectable, date: Date = new Date()) {
  ...
  return contactsForUser(user, ({ birthday: { date } }) => {
